...
// const { idType, namingConvention = 'hasura-default' } =
const { idType, namingConvention = 'hasura-default' } =
  {} as HasuraDataProviderOptions;
...
const hasuraDataProvider: Required<DataProvider> = {
  getOne: async ({ resource, id, meta }) => {
    ...
    throw new Error('Operation queries not implemented');
  ...
  deleteOne: async ({ resource, id, meta }) => {
    ...
    console.log('deleteOne', { deleteOperation, resource, id, meta });
    if (meta?.gqlMutation) {
      const response = await gqlClient.request<BaseRecord>({
        document: meta.gqlMutation,
        variables: {
