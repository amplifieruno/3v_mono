...
import { DeviceInsertOneMutation, AreasWithFacilityQuery } from '../../queries';
...
export const CreatePage: FC = () => {
  ...
  const {
    ...
    defaultValues: {
      name: '',
      device_type: 'static_video_camera',
      area_id: '',
      stream_url: '',
      resolution: '',
      fps: '',
    ...
    refineCoreProps: {
      meta: { gqlMutation: DeviceInsertOneMutation },
    },
  });
  ...
  const handleFormSubmit = handleSubmit((values) => {
    ...
    if (!data.stream_url) delete data.stream_url;
    if (!data.resolution) delete data.resolution;
    if (!data.fps) delete data.fps;
    else data.fps = Number(data.fps);
    if (data.credentials) {
      try {
        data.credentials = JSON.parse(data.credentials as string);
      } catch {
    ...
    if (data.configuration) {
      try {
        data.configuration = JSON.parse(data.configuration as string);
      } catch {
